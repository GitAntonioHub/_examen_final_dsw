<div>
    ...
    @include('livewire.partials.scenario-header')
    
    <div class="row">
        <div>
            <h1>Simulación del Escenario: {{ $scenario->name ?? 'N/A' }}</h1>
            <p>Desa Trainer: {{ $desaTrainers->name ?? 'N/A' }}</p>
            <h3>Instrucciones:</h3>
            <ul>
                ...
                <div class="card-body p-0">
                    ...
                    <div class="position-relative d-flex justify-content-center" wire:ignore>
                        ...
                        <div id="desaContainer" class="w-60 position-relative">
                            ...
                            @if($desaTrainers)
                                <img src="{{ asset('storage/' . $desaTrainers->image) }}" class="w-100" alt="{{ $desaTrainers->name }}">
                            @else
